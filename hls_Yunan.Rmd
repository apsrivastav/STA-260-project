...
```
###  the distribution plots for each of the variables 

```{r BYSEX}
library(ggplot2)

#math
ggplot(fdata, aes(x = factor(BYSEX), y = BYTXMSTD  , fill = factor(BYSEX))) +
  geom_boxplot() +
  scale_fill_brewer(palette = "Pastel2")

fdata$BYSEX = factor(fdata$BYSEX)
ggplot(fdata, aes(x = BYTXMSTD, fill =BYSEX )) +
  geom_density(alpha = 0.3)

#read
ggplot(fdata, aes(x = factor(BYSEX), y = BYTXRSTD    , fill = factor(BYSEX))) +
  geom_boxplot() +
  scale_fill_brewer(palette = "Pastel2")

fdata$BYSEX = factor(fdata$BYSEX)
ggplot(fdata, aes(x = BYTXRSTD  , fill =BYSEX )) +
  geom_density(alpha = 0.3)

#different gender students' math and read performance are alike.
```
```{r BYSTLANG}
#math
ggplot(fdata, aes(x = factor(BYSTLANG), y = BYTXMSTD  , fill = factor(BYSTLANG  ))) +
  geom_boxplot() +
  scale_fill_brewer(palette = "Pastel2")

fdata$BYSTLANG = factor(fdata$BYSTLANG)
ggplot(fdata, aes(x = BYTXMSTD, fill =BYSTLANG )) +
  geom_density(alpha = 0.3)

#read
ggplot(fdata, aes(x = factor(BYSTLANG), y = BYTXRSTD    , fill = factor(BYSTLANG))) +
  geom_boxplot() +
  scale_fill_brewer(palette = "Pastel2")

ggplot(fdata, aes(x = BYTXRSTD  , fill =BYSTLANG )) +
  geom_density(alpha = 0.3)

#English native students tend to have better math and read score, especially reading
```
```{r BYFCOMP}
#math
ggplot(fdata, aes(x = factor(BYFCOMP), y = BYTXMSTD  , fill = factor(BYFCOMP))) +
  geom_boxplot() +
  scale_fill_brewer(palette = "Spectral")

fdata$BYFCOMP= factor(fdata$BYFCOMP)
ggplot(fdata, aes(x = BYTXMSTD, fill =BYFCOMP)) +
  geom_density(alpha = 0.3)

#read
ggplot(fdata, aes(x = factor(BYFCOMP), y = BYTXRSTD, fill = factor(BYFCOMP))) +
  geom_boxplot() +
  scale_fill_brewer(palette = "Spectral")

ggplot(fdata, aes(x = BYTXRSTD, fill =BYFCOMP )) +
  geom_density(alpha = 0.3)

#students have both mother and father tend to have better math and read score
#student from a Father only family have better m&r score compare to other untraditational family
```
```{r BYSIBHOM}
#math
ggplot(fdata, aes(x = factor(BYSIBHOM), y = BYTXMSTD  , fill = factor(BYSIBHOM))) +
  geom_boxplot() +
  scale_fill_brewer(palette = "Spectral")

fdata$BYSIBHOM= factor(fdata$BYSIBHOM  )
ggplot(fdata, aes(x = BYTXMSTD, fill =BYSIBHOM)) +
  geom_density(alpha = 0.3)

#read
ggplot(fdata, aes(x = factor(BYSIBHOM), y = BYTXRSTD, fill = factor(BYSIBHOM))) +
  geom_boxplot() +
  scale_fill_brewer(palette = "Spectral")

ggplot(fdata, aes(x = BYTXRSTD, fill =BYSIBHOM )) +
  geom_density(alpha = 0.3)

#students with 0-2 siblings tend to have better math and read score
# for students with more than siblings,siblings number seems have negtive relation with m&r grade 
```
```{r BYPARED   }
#math
ggplot(fdata, aes(x = factor(BYPARED), y = BYTXMSTD  , fill = factor(BYPARED))) +
  geom_boxplot() +
  scale_fill_brewer(palette = "Spectral")

fdata$BYPARED= factor(fdata$BYPARED  )
ggplot(fdata, aes(x = BYTXMSTD, fill =BYPARED)) +
  geom_density(alpha = 0.3)

#read
ggplot(fdata, aes(x = factor(BYPARED), y = BYTXRSTD, fill = factor(BYPARED))) +
  geom_boxplot() +
  scale_fill_brewer(palette = "Spectral")

ggplot(fdata, aes(x = BYTXRSTD, fill =BYPARED )) +
  geom_density(alpha = 0.3)

#Parents’ educational level is very likely to be related to their children’s grades

```
```{r BYINCOME}
#math
ggplot(fdata, aes(x = factor(BYINCOME), y = BYTXMSTD  , fill = factor(BYINCOME))) +
  geom_boxplot() +
  scale_fill_brewer(palette = "Spectral")

fdata$BYINCOME= factor(fdata$BYINCOME  )
ggplot(fdata, aes(x = BYTXMSTD, fill =BYINCOME)) +
  geom_density(alpha = 0.3)

#read
ggplot(fdata, aes(x = factor(BYINCOME), y = BYTXRSTD, fill = factor(BYINCOME))) +
  geom_boxplot() +
  scale_fill_brewer(palette = "Spectral")

ggplot(fdata, aes(x = BYTXRSTD, fill =BYINCOME )) +
  geom_density(alpha = 0.3)

#Total family income is very likely to be related to their children’s grades

```
```{r BYGRDRPT  }
#math
ggplot(fdata, aes(x = factor(BYGRDRPT), y = BYTXMSTD  , fill = factor(BYGRDRPT))) +
  geom_boxplot() +
  scale_fill_brewer(palette = "Spectral")

fdata$BYGRDRPT= factor(fdata$BYGRDRPT  )
ggplot(fdata, aes(x = BYTXMSTD, fill =BYGRDRPT)) +
  geom_density(alpha = 0.3)

#read
ggplot(fdata, aes(x = factor(BYGRDRPT), y = BYTXRSTD, fill = factor(BYGRDRPT))) +
  geom_boxplot() +
  scale_fill_brewer(palette = "Spectral")

ggplot(fdata, aes(x = BYTXRSTD, fill =BYGRDRPT )) +
  geom_density(alpha = 0.3)

#the number of grades in which the sample member was held back between Kindergarten and 10th grade.

#Previous education experience is related to grade 12 grades, students who repeat less tend to have higher grades

```

```{r BYNONUSG  }
#math
ggplot(fdata, aes(x = factor(BYNONUSG), y = BYTXMSTD  , fill = factor(BYNONUSG))) +
  geom_boxplot() +
  scale_fill_brewer(palette = "Spectral")

fdata$BYNONUSG= factor(fdata$BYNONUSG  )
ggplot(fdata, aes(x = BYTXMSTD, fill =BYNONUSG)) +
  geom_density(alpha = 0.3)

#read
ggplot(fdata, aes(x = factor(BYNONUSG), y = BYTXRSTD, fill = factor(BYNONUSG))) +
  geom_boxplot() +
  scale_fill_brewer(palette = "Spectral")

ggplot(fdata, aes(x = BYTXRSTD, fill =BYNONUSG )) +
  geom_density(alpha = 0.3)

#Number of grades completed outside U.S. (K-10) 
#kids spend 8-10 grades outsides may have difficulty adapting to the new environment, which affects overall grades

```
```{r BYSCHPRG}
#math
ggplot(fdata, aes(x = factor(BYSCHPRG), y = BYTXMSTD  , fill = factor(BYSCHPRG))) +
  geom_boxplot() +
  scale_fill_brewer(palette = "Spectral")

fdata$BYSCHPRG= factor(fdata$BYSCHPRG  )
ggplot(fdata, aes(x = BYTXMSTD, fill =BYSCHPRG)) +
  geom_density(alpha = 0.3)

#read
ggplot(fdata, aes(x = factor(BYSCHPRG), y = BYTXRSTD, fill = factor(BYSCHPRG))) +
  geom_boxplot() +
  scale_fill_brewer(palette = "Spectral")

ggplot(fdata, aes(x = BYTXRSTD, fill =BYSCHPRG )) +
  geom_density(alpha = 0.3)
...
# I am not familiar with this variable
# seems College preparatory-academic programe can help students get better score 
# or only the students in good study condition can be included in the College preparatory-academic programe
```
```{r BYTXACC}
#math
ggplot(fdata, aes(x = factor(BYTXACC), y = BYTXMSTD  , fill = factor(BYTXACC))) +
  geom_boxplot() +
  scale_fill_brewer(palette = "Spectral")

fdata$BYTXACC= factor(fdata$BYTXACC  )
ggplot(fdata, aes(x = BYTXMSTD, fill =BYTXACC)) +
  geom_density(alpha = 0.3)

#read
ggplot(fdata, aes(x = factor(BYTXACC), y = BYTXRSTD, fill = factor(BYTXACC))) +
  geom_boxplot() +
  scale_fill_brewer(palette = "Spectral")

ggplot(fdata, aes(x = BYTXRSTD, fill =BYTXACC )) +
  geom_density(alpha = 0.3)

#students who need Extra time or other test accommodations are less likely to get a better score. 
# I am not familiar with this variable too in what situation students can ask for Extra time or other test accommodations?

```
```{r BYWRKHRS}
#math
ggplot(fdata, aes(x = factor(BYWRKHRS), y = BYTXMSTD  , fill = factor(BYWRKHRS))) +
  geom_boxplot() +
  scale_fill_brewer(palette = "Spectral")

fdata$BYWRKHRS= factor(fdata$BYWRKHRS  )
ggplot(fdata, aes(x = BYTXMSTD, fill =BYWRKHRS)) +
  geom_density(alpha = 0.3)

#read
ggplot(fdata, aes(x = factor(BYWRKHRS), y = BYTXRSTD, fill = factor(BYWRKHRS))) +
  geom_boxplot() +
  scale_fill_brewer(palette = "Spectral")

ggplot(fdata, aes(x = BYTXRSTD, fill =BYWRKHRS )) +
  geom_density(alpha = 0.3)

#BY hours worked per week during 01-02 school year 
# 1-5 hours a week is a good pratice
# more than 26 hours a week will be a heavy burden


```
```{r BYTEHDEG}

fdata$BYTEHDEG= factor(fdata$BYTEHDEG  )
#read
ggplot(fdata, aes(x = factor(BYTEHDEG), y = BYTXRSTD, fill = factor(BYTEHDEG))) +
  geom_boxplot() +
  scale_fill_brewer(palette = "Spectral")

ggplot(fdata, aes(x = BYTXRSTD, fill =BYTEHDEG )) +
  geom_density(alpha = 0.3)

#Highest degree earned by the English teacher
# No Degree and Associate degree (A.A., A.S., etc.) is quite rare
# higher the Degree higher the read score


```
```{r BYTMHDEG    }
#math
ggplot(fdata, aes(x = factor(BYTMHDEG), y = BYTXMSTD  , fill = factor(BYTMHDEG))) +
  geom_boxplot() +
  scale_fill_brewer(palette = "Spectral")

fdata$BYTMHDEG= factor(fdata$BYTMHDEG  )
ggplot(fdata, aes(x = BYTXMSTD, fill =BYTMHDEG)) +
  geom_density(alpha = 0.3)

#Highest degree earned by the math teacher
# No Degree and Associate degree (A.A., A.S., etc.) is quite rare,too
# no clear pattern between teacher's Degree vs student's math score


```
```{r BYSCTRL}
#math
ggplot(fdata, aes(x = factor(BYSCTRL), y = BYTXMSTD  , fill = factor(BYSCTRL))) +
  geom_boxplot() +
  scale_fill_brewer(palette = "Spectral")

fdata$BYSCTRL= factor(fdata$BYSCTRL  )
ggplot(fdata, aes(x = BYTXMSTD, fill =BYSCTRL)) +
  geom_density(alpha = 0.3)

#read
ggplot(fdata, aes(x = factor(BYSCTRL), y = BYTXRSTD, fill = factor(BYSCTRL))) +
  geom_boxplot() +
  scale_fill_brewer(palette = "Spectral")

ggplot(fdata, aes(x = BYTXRSTD, fill =BYSCTRL )) +
  geom_density(alpha = 0.3)

#Public, Catholic, or Other Private School 
#Public school performance is generally worse than the others
```
```{r BYURBAN}
#math
i= BYURBAN
fdata$BYURBAN= factor(fdata$BYURBAN)
ggplot(fdata, aes(x = factor(i), y = BYTXMSTD  , fill = factor(i))) +
  geom_boxplot() +
  scale_fill_brewer(palette = "Spectral")

ggplot(fdata, aes(x = BYTXMSTD, fill =i)) +
  geom_density(alpha = 0.3)

#read
ggplot(fdata, aes(x = factor(i), y = BYTXRSTD, fill = factor(i))) +
  geom_boxplot() +
  scale_fill_brewer(palette = "Spectral")

ggplot(fdata, aes(x = BYTXRSTD, fill =i )) +
  geom_density(alpha = 0.3)

#Urbanicity of school locale 
#different school local with alike math and read performance
```
```{r BYREGION  }
#math

fdata$BYREGION= factor(fdata$BYREGION)

ggplot(fdata, aes(x = factor(BYREGION), y = BYTXMSTD  , fill = factor(BYREGION))) +
  geom_boxplot() +
  scale_fill_brewer(palette = "Spectral")

ggplot(fdata, aes(x = BYTXMSTD, fill =BYREGION)) +
  geom_density(alpha = 0.3)

#read
ggplot(fdata, aes(x = factor(BYREGION), y = BYTXRSTD, fill = factor(BYREGION))) +
  geom_boxplot() +
  scale_fill_brewer(palette = "Spectral")

ggplot(fdata, aes(x = BYTXRSTD, fill =BYREGION )) +
  geom_density(alpha = 0.3)

#1 Northeast 2 Midwest 3 South 4 West
#different Geographic region school with alike math and read performance
# Northeast school has a weak advantage
```
```{r BYAPIB}
#math

fdata$BYAPIB= factor(fdata$BYAPIB)

ggplot(fdata, aes(x = factor(BYAPIB), y = BYTXMSTD  , fill = factor(BYAPIB))) +
  geom_boxplot() +
  scale_fill_brewer(palette = "Spectral")

ggplot(fdata, aes(x = BYTXMSTD, fill =BYAPIB)) +
  geom_density(alpha = 0.3)

#read
ggplot(fdata, aes(x = factor(BYAPIB), y = BYTXRSTD, fill = factor(BYAPIB))) +
  geom_boxplot() +
  scale_fill_brewer(palette = "Spectral")

ggplot(fdata, aes(x = BYTXRSTD, fill =BYAPIB )) +
  geom_density(alpha = 0.3)

# Ever in Advanced Placement program
# I am not familiar with this variable
# seems Advanced Placement program are like College preparatory-academic programe, can help students get better score 
# or only the students in good study condition can be included in the Advanced Placement program?
```
```{r BYERCEMTCH}
#math

fdata$BYERCEMTCH= factor(fdata$BYERCEMTCH)

ggplot(fdata, aes(x = factor(BYERCEMTCH), y = BYTXMSTD  , fill = factor(BYERCEMTCH))) +
  geom_boxplot() +
  scale_fill_brewer(palette = "Spectral")

ggplot(fdata, aes(x = BYTXMSTD, fill =BYERCEMTCH)) +
  geom_density(alpha = 0.3)

#read
ggplot(fdata, aes(x = factor(BYERCEMTCH), y = BYTXRSTD, fill = factor(BYERCEMTCH))) +
  geom_boxplot() +
  scale_fill_brewer(palette = "Spectral")

ggplot(fdata, aes(x = BYTXRSTD, fill =BYERCEMTCH )) +
  geom_density(alpha = 0.3)
...
fdata$BYHMWRK= factor(fdata$BYHMWRK)

ggplot(fdata, aes(x = factor(BYHMWRK), y = BYTXMSTD  , fill = factor(BYHMWRK))) +
  geom_boxplot() 

ggplot(fdata, aes(x = BYTXMSTD, fill =BYHMWRK)) +
  geom_density(alpha = 0.3)

#read
ggplot(fdata, aes(x = factor(BYHMWRK), y = BYTXRSTD, fill = factor(BYHMWRK))) +
  geom_boxplot() 

ggplot(fdata, aes(x = BYTXRSTD, fill =BYHMWRK )) +
  geom_density(alpha = 0.3)

# BY hours per week spent on homework (in and out of school)
# one week has 168 hours
# spent 99 hours on homework??? poor kids or poor data
```
